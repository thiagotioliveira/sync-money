...
public class TransactionServiceImpl implements TransactionService {
  ...
  public Transfer transfer(CreateTransferInput input) {
    return this.domainEventContext.execute(() -> this.createTransfer.execute(input));
  }

  @Override
  public Transaction createDeposit(CreateTransactionInput input) {
    return this.domainEventContext.execute(() -> this.createTransaction.deposit(input));
  }

  @Override
  public Transaction createWithdraw(CreateTransactionInput input) {
    return this.domainEventContext.execute(() -> this.createTransaction.withdraw(input));
  }

  @Override
  public PayableReceivable createPayable(CreatePayableReceivableInput input) {
    return this.domainEventContext.execute(() -> this.createPayableReceivable.payable(input));
  }

  @Override
  public PayableReceivable createReceivable(CreatePayableReceivableInput input) {
    return this.domainEventContext.execute(() -> this.createPayableReceivable.receivable(input));
  }

  @Override
  public Transaction update(
      UUID organizationId, UUID accountId, UUID transactionId, UpdateTransactionInput input) {
    return this.domainEventContext.execute(
  ...
  public Transaction pay(PayTransactionInput input) {
    return this.domainEventContext.execute(() -> this.updateTransaction.pay(input));
  }
  ...
  public List<TransactionEnriched> getByAccountIds(
    ...
    return this.domainEventContext.execute(
        () -> this.getTransaction.byAccountIdsAndYearMonth(organizationId, accountIds, yearMonth));
  }
}
